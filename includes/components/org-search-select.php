        ...
        <div x-transition class="rounded-150 bg-white border flex items-center flex-col p-5">
            <div class="flex w-full justify-end mb-4">
                <button x-on:click.prevent="showingRemoveConfirmation = false"
                    class="font-semibold"><?php _e('Close X', 'wicket') ?></button>
            </div>
...
?>
    ...
    <div class="orgss-search-form flex flex-col bg-dark-100 bg-opacity-5 rounded-100 p-3">
      ...
      if(empty($title)) : ?>
            <h2
                class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__current-orgs-title' : 'font-bold text-body-lg my-3 orgss-search-form__title' ?>">
            ...
            <template x-for="(connection, index) in currentConnections" :key="connection.connection_id" x-transition>
                ...
                && connection.active_connection" <?php if (defined('WICKET_WP_THEME_V2')) : ?>
                    ...
                    '' "
                    <?php else : ?>
                    class="rounded-100 flex flex-col md:flex-row md:justify-between bg-white p-4 mb-3"
                    x-bind:class="connection.org_id == selectedOrgUuid ? 'border-success-040 border-opacity-100
                    border-4' : 'border border-dark-100 border-opacity-5' "
                    <?php endif; ?>
                    >
            ...
            if(connection.org_parent_name) {
              if(connection.org_parent_name !== null) {
                ...
                orgParentName = connection.org_parent_name;
                        ...
                        <div class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__org-type' : 'font-bold text-body-xs' ?>"
                            x-text="connection.org_type_name"></div>
                        <div
                            class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__card-top-header' : 'flex flex-col sm:flex-row mb-2 sm:items-center' ?>">
                            <div x-text="connection.org_name"
                                class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__org-name' : 'font-bold text-body-sm sm:mr-5' ?>">
                            </div>
                            <div>
                                ...
                                <template x-if="! connection.active_membership">
                                    <div
                                        class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__inactive-membership-label' : '' ?>">
                                        <i
                        ...
                        <div x-show="showOrgParentName"
                            class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__org-parent-name' : 'mb-3' ?>"
                            x-text="orgParentName"></div>
                        <div x-text="description"
                            class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__org-description' : '' ?>">
                        </div>
                    </div>
                    <div
                        class="current-org-listing-right <?php echo defined('WICKET_WP_THEME_V2') ? '' : 'flex flex-col sm:flex-row items-start sm:items-center gap-2 mt-4 md:mt-0' ?>">
                        <?php get_component('button', [
      'variant'  => 'secondary',
      'reversed' => false,
      ...
      'atts'     => [ 'x-on:click.prevent="terminateRelationship($data.connection.connection_id)"',
        ...
        <div class="<?php echo defined('WICKET_WP_THEME_V2') ? 'label' : 'font-bold text-body-md mb-2' ?>"
        ...
        <div class="flex">
            ...
            <input x-model="searchBox" @keydown.enter.prevent.stop="handleSearch()" type="text"
                class="orgss-search-box w-full mr-2"
...
'atts'  => [ 'x-on:click.prevent="handleSearch()"' ],
        ...
        <div class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__matching-orgs-title' : 'mt-4 mb-1' ?>"
            x-show="firstSearchSubmitted || isLoading" x-cloak>
        ...
        <div class="orgss-results" x-bind:class="results.length == 0 ? '' : 'orgss-results--has-results' ">
            <div
                class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__search-container' : 'flex flex-col bg-white px-4 max-h-80 overflow-y-scroll' ?>">
                ...
                <template x-for="(result, uuid) in results" x-cloak>
                    <div
                        ...
                        <div class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__matching-org-title' : 'font-bold' ?>"
    ...
    <div x-show="firstSearchSubmitted && !disableCreateOrgUi" x-cloak
        class="orgss-create-org-form <?php echo defined('WICKET_WP_THEME_V2') ? '' : 'mt-4 flex flex-col bg-dark-100 bg-opacity-5 rounded-100 p-3' ?>">
        <div
            class="<?php echo defined('WICKET_WP_THEME_V2') ? 'component-org-search-select__cant-find-org-title' : 'font-bold text-body-md mb-2' ?>">
            <?php _e('Can\'t find your', 'wicket') ?>
            <?php echo $orgTermSingularLower; ?>?
        </div>
        <div class="flex">
            <div x-bind:class="newOrgTypeOverride.length <= 0 ? 'w-5/12' : 'w-10/12'" class="flex flex-col mr-2">
                <label><?php _e('Name of the', 'wicket') ?>
                    <?php echo $orgTermSingularCap; ?>*</label>
                <input x-model="newOrgNameBox" @keyup.enter.prevent.stop="handleOrgCreate($event)" type="text"
                    name="company-name" class="w-full" />
            </div>
            <div x-show="newOrgTypeOverride.length <= 0" class="flex flex-col w-5/12 mr-2">
                <label><?php _e('Type of', 'wicket') ?>
                    <?php echo $orgTermSingularCap; ?>*</label>
    ...
    'atts'  => [ 'x-on:click.prevent="handleOrgCreate($event)"' ],
        ...
        <div x-show="displayDuplicateOrgWarning"
            class="orgss-red-alert flex mt-4 p-4 border-solid border-l-4 border-t-0 border-r-0 border-b-0 <?php echo defined('WICKET_WP_THEME_V2') ? 'bg-[--state-error-light] border-[--state-error]' : 'bg-[#f5c2c7] border-[#dc3545]' ?>">
      ...
      if (defined('WICKET_WP_THEME_V2')) {
          ...
          get_component('icon', [
            ...
            <div class="text-col">
                <div class="font-bold text-body-lg">
